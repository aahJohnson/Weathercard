...
public class weatherImport {
	...
	public static void getWeather(weatherBean wBean) throws IOException {
		...
		String URLtoSend = "http://api.openweathermap.org/data/2.5/weather?q=" + wBean.getCityName() + ","
				+ wBean.getCountryCode() + "&APPID=a5fdd6a91783432c3c7cca8aa5aa7ace&mode=xml";
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				// and print it
				System.out.println(wBean.getCityName() + " is now a " + XMLclouds);
				// save it
				wBean.setCloudPattern(XMLclouds);
